 ...
 * - onPropsChange 在 props 变化重渲染后调用
 *
 * 扁平化 props 协议：宿主把每个 prop 作为独立 kebab-case attribute 传入
 * （timezone、label），包装层收集后作为扁平 props 对象传给各回调。
 *
...
export default {
  ...
  /**
   * 渲染函数：返回 HTML 字符串
   * @param {object} props 物料扁平化 props（由包装层从 kebab-case attribute 收集并按类型解析）
   * @param {object} scope widgetScope 软隔离对象
   * @returns {string} HTML 字符串
   */
  render(props, scope) {
    const timezone = props.timezone || 'UTC';
    const label = props.label || scope.meta.name;
  ...
  onPropsChange(element, newProps, oldProps, scope) {
    if (newProps.timezone !== oldProps.timezone) {
      scope.log.info('时区切换:', oldProps.timezone, '->', newProps.timezone);
    }
  }
};
