...
test.describe('体积优化验证', () => {
  ...
  test('Vue2 物料产物 < 3KB', () => {
    const jsSize = getFileSizeKB(path.join(DEMO_DIR, 'vue2-widget/dist/widget.js'));
    const cssSize = getFileSizeKB(path.join(DEMO_DIR, 'vue2-widget/dist/style.css'));

    console.log(`Vue2 widget.js: ${jsSize.toFixed(2)} KB`);
    ...
    expect(jsSize + cssSize).toBeLessThan(3.5);
  });

  test('Vue3 物料产物 < 4KB', () => {
    const jsSize = getFileSizeKB(path.join(DEMO_DIR, 'vue3-widget/dist/widget.js'));
    const cssSize = getFileSizeKB(path.join(DEMO_DIR, 'vue3-widget/dist/style.css'));

    console.log(`Vue3 widget.js: ${jsSize.toFixed(2)} KB`);
    ...
    expect(jsSize + cssSize).toBeLessThan(4.5);
  });

  test('H5 物料产物 < 1KB', () => {
    const jsSize = getFileSizeKB(path.join(DEMO_DIR, 'h5-widget/dist/widget.js'));

    console.log(`H5 widget.js: ${jsSize.toFixed(2)} KB`);

    expect(jsSize).toBeLessThan(1);
  });

  test('三个物料总产物 < 4KB', () => {
    const vue2Js = getFileSizeKB(path.join(DEMO_DIR, 'vue2-widget/dist/widget.js'));
    const vue2Css = getFileSizeKB(path.join(DEMO_DIR, 'vue2-widget/dist/style.css'));
    const vue3Js = getFileSizeKB(path.join(DEMO_DIR, 'vue3-widget/dist/widget.js'));
    const vue3Css = getFileSizeKB(path.join(DEMO_DIR, 'vue3-widget/dist/style.css'));
    const h5Js = getFileSizeKB(path.join(DEMO_DIR, 'h5-widget/dist/widget.js'));
