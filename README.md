...
- **看板基座（Host）**：负责统一渲染、按需加载、版本治理、错误隔离、跨物料通信。
- **物料组件库（Widget Libs）**：各部门继续用熟悉的 Vue2 或 Vue3 开发业务组件，通过打包插件将其输出为标准的 Custom Element（UMD 格式）。

核心目标是不强制统一技术栈、业务组件零改造、按需加载、公共依赖复用。
